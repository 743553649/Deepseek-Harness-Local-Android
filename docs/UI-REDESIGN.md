...
> **状态**：**已实现并验收通过**（v1.2.36 ~ v1.2.43）。⚠️ 实现过程中用户又改了几处设计，
...
> ⚠️ **本节是当初的定稿形态**（三格导航 + 174×45dp 悬浮岛）。实际落地是**四格 + 底栏铺满 + 全屏**，
> 逐条差异见 **§9.2**；下面的图与说明仅作设计理由留存。
...
> ⚠️ 颜色 / 字体 / 动效令牌仍然有效；**与底栏尺寸有关的那几行已变**（玻璃岛 174×45dp → 铺满整宽 64dp、
> 药丸 39dp → 居中胶囊、玻璃透明度 72%→54% 改成 44%→24%），以 §9.2 为准。
...
**先推 main 走 CI**（约 2-3 分钟），拉产物装机，然后**逐项确认**（下表为 v1.2.43 的最终结果）：
...
> **验收结论（v1.2.43）**：用户整体验收通过。下面两项**没有专门实测**（不是"测过没问题"），
> 写在这里免得以后误以为已覆盖：
> - 横屏 + 桌面布局：底栏是整宽 + weight 布局，逻辑上不存在"岛宽"问题；横屏若发现问题回来补
> - 系统「减少动态效果」开关：所有动画先过 `Motion.reduced()`，但没在真机上开过那个开关验
...
```
┌──────────────────────────────────┐  ← 状态栏区域也被 App 覆盖（透明状态栏，
│  WebView（引擎对话网页）            │     显示的其实是页面自己的底色）
│                                  │
│                                  │
│  …对话内容…                       │
│  ┌────────────────────────────┐  │
