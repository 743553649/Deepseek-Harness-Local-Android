...
**常驻通知与彻底退出**
引擎状态常驻在通知栏（`正在启动引擎…` / `引擎已就绪` / 引擎反复起不来时显示原因），点通知直接回到 App。
点通知里的「退出」会**彻底退出**（引擎优雅停完 → 关掉 App 进程 → 连最近任务里的卡片一起撤掉，
不会留个空壳挂在后台或最近任务里）；点「退出」后马上重开 App 也不会再报错。
...
|---|---|---|
| **本文件** | 使用者 | 产品说明、安装、FAQ |
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | 改代码的人 / Agent | 代码导航：哪个文件负责什么、引擎启动链路、端口与目录布局、三条数据流、CI 流水线、与上游的差异清单 |
| [`docs/PITFALLS.md`](docs/PITFALLS.md) | 改代码的人 / Agent | 踩坑记录（A~J 节，全部按「现象 → 根因 → 修法 → 验证」写）+ 真机验证配方 |
